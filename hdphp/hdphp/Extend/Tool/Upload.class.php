...
class Upload
{
    ...
    /**
     * 将$_FILES中的文件上传到服务器
     * @param null $filesData 某一个上传文件数据
     * @return array|bool
     */
    public function upload($filesData = null)
    {
        if (!$this->checkDir($this->path)) {
            $this->error = $this->path . L("upload_upload_error");
            return false;
        }
        $files = $this->format($filesData);
        //验证文件
        foreach ($files as $v) {
    ...
    private function save($file)
    {
        ...
        if (C("UPLOAD_IMG_RESIZE_ON")) {
            $img->thumb($filePath,  C("UPLOAD_IMG_MAX_WIDTH"), C("UPLOAD_IMG_MAX_HEIGHT"), 5,"",$uploadFileName);
        }
        //生成缩略图
        if ($this->thumbOn) {
            $args = array();
